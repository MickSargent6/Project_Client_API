...
begin
  result:=False;
  bi:=Block;
  if (bi = nil) or (bi^.Counter <> 0) then exit;
  oi:=InstFreeList;
  poi:=nil;
  while (oi <> nil) do
  begin
...
begin
  ...
  begin
     ...
     begin
        if Assigned(pbi) then pbi^.Next:=nbi;
     ...
     try
        ...
        begin
           ...
           begin
              ...
              Dec(bi^.Counter);
              if (bi^.Counter <= 0) then FreeInstanceBlocks;
           end;
        end;
     finally
        LeaveCriticalSection(InstCritSect);
...
begin
  ...
  begin
     if ClassReg then Windows.UnregisterClass(ObjWndClass.lpszClassName, HInstance);
...
//  {$ifdef debug}
 // DebugPoint('objinst.finalization');
 // {$endif}
end.
