...
type
  ...
  protected
     ...
     //
     Procedure      DoMessage (Sender: TObject; Pipe: HPIPE; Stream: TStream); virtual;
  public
     // Public declarations
  ...
  published
     ...
     property       OnPipeSent: TOnPipeSent read FOPS write FOPS;
     property       OnPipeError: TOnPipeError read FOPE write FOPE;
     Property       OnPipeConnect: TOnPipeConnect read fOnPipeConnect write fOnPipeConnect;
  end;
...
begin
  ...
  FConnected:=result;
  if fConnected then
    if Assigned (fOnPipeConnect) then fOnPipeConnect (Self, fPipe);
end;
...
Procedure TPipeClient.DoMessage(Sender: TObject; Pipe: HPIPE; Stream: TStream);
begin
  if Assigned(FOPM) then FOPM (Sender, Pipe, Stream);
end;
...
begin
  ...
  if FConnected then raise EPipeException.CreateRes(@resPipeConnected);
...
begin
  ...
  case Message.Msg of
     ...
     WM_PIPEDISCONNECT :  if Assigned(FOPD) then FOPD(Self, Message.wParam);
     ...
     begin
        ...
        begin
           ...
           try
              ...
              DoMessage (Self, Message.wParam, MemStream);
           finally
              MemStream.Free;
...
begin
  ...
  case Message.Msg of
     WM_QUERYENDSESSION:  Message.Result:=1;
     WM_PIPEERROR_L    :  if not(FInShutdown) and Assigned(FOPE) then FOPE(Self, Message.wParam, pcListener, Message.lParam);
     WM_PIPEERROR_W    :  if not(FInShutdown) and Assigned(FOPE) then FOPE(Self, Message.wParam, pcWorker, Message.lParam);
...
begin
  ...
  begin
     ...
     if (ppiClient^.Pipe = Pipe) then break;
     ppiClient:=nil;
  end;
...
begin
  ...
  if FActive then raise EPipeException.CreateRes(@resPipeActive);
...
begin
  ...
  if (not(FInShutDown) and
     (FThreadCount = 0)) then FActive:=False;
  end;
...
Procedure TPipeServer.DoMessage(Sender: TObject; Pipe: HPIPE; Stream: TStream);
begin
  if Assigned(FOPM) then FOPM (Sender, Pipe, Stream);
end;
...
begin
  ...
  try
     ...
     begin
        ...
        if (GetTickCount > failsafe+10000) then break;
     end;
  finally
...
begin
  ...
  begin
     ...
     begin
        // Call OnError in the main thread if this is not a disconnect. Disconnects
        // have their own event, and are not to be considered an error
        if (FErrorCode <> ERROR_BROKEN_PIPE) then PostMessage(FNotify, WM_PIPEERROR_W, FPipe, FErrorCode);
        // Terminate
        Terminate;
     end;
  end;
...
begin
  ...
  begin
     if FConnected then DisconnectNamedPipe(FPipe);
     CloseHandle(FPipe);
  end;
...
begin
  ...
  begin
     ...
     begin
        ...
        // pipe server
        if (FErrorCode <> ERROR_NO_DATA) then PostMessage(FNotify, WM_PIPEERROR_L, FPipe, FErrorCode);
        // Close the handle
        CloseHandle(FPipe);
        FPipe:=INVALID_HANDLE_VALUE;
     end
...
begin
  ...
  if (FCount = 0) then ResetEvent(FDataEv);
